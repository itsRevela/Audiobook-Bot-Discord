...
def combine_chapters_to_m4b():
    ...
    final_output_path = os.path.normpath(final_output_path) # <<<--- THIS IS THE FIX
    ...
    try:
        with open(concat_list_filename, 'w', encoding='utf-8') as concat_list_file:
            for i, mp3_file in enumerate(mp3_files):
                safe_path = os.path.normpath(mp3_file).replace("\\", "/")
        ...
        for line in process.stdout:
            ...
            if time_match:
                ...
                percent = (elapsed_seconds / total_duration) * 100 if total_duration > 0 else 0
                bar_length = 30
                filled_len = int(bar_length * elapsed_seconds // total_duration) if total_duration > 0 else 0
